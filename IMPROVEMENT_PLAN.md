...
- [ ] **Fix `package.json`**: rename `services-dasboard` → `services-dashboard`; remove or repoint the `db:generate` / `db:migrate` scripts (MongoDB connector has no migrate engine — keep `db:push` only).
...
- [ ] **`users-table.tsx` (272 → ~120 lines)**: extract `user-actions-menu.tsx` (the `Menu.Root` block, lines 197-249) and move sorting to the Phase-2 hook; keep dialogs' state in the table.
